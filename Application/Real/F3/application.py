...
def show_result():
    ...
    ax1.imshow(seismic_raw.T, aspect=1.5, cmap=plt.cm.gray, vmin=-17000, vmax=16000)
    ax1.set_ylabel("Samples", font1)
    ax1.set_title("Traces", font1)
    ax1.text(-12, -7, "(a)", font1)

    ax2 = fig.add_subplot(gs[0:20, 32:62])
    ax2.imshow(seismic_raw.T, aspect=1.5, cmap=plt.cm.gray, vmin=-17000, vmax=16000)
    ax2.imshow(fault_raw, aspect=1.5, cmap="jet", vmin=0.15, vmax=0.85, alpha=raw_alpha)
    ax2.text(-12, -7, "(b)", font1)
    ax2.set_title("Traces", font1)

    ax3 = fig.add_subplot(gs[24:44, 0:30])
    ax3.imshow(seismic_sr.T, aspect=1.5, cmap=plt.cm.gray, vmin=-17000, vmax=16000)
    ax3.set_ylabel("Samples", font1)
    ax3.text(-24, -14, "(c)", font1)

    ax4 = fig.add_subplot(gs[24:44, 32:62])
    ax4.imshow(seismic_sr.T, aspect=1.5, cmap=plt.cm.gray, vmin=-17000, vmax=16000)
    ax4.imshow(fault_sr, aspect=1.5, cmap="jet", vmin=0.15, vmax=0.85, alpha=sr_alpha)
    ax4.text(-24, -14, "(d)", font1)

    # define colorbar
    dx1 = fig.add_subplot(gs[46:47, 0:30])
    norm1 = mpl.colors.Normalize(vmin=-17000, vmax=16000)
    dbar1 = fig.colorbar(mpl.cm.ScalarMappable(norm=norm1, cmap=plt.cm.gray),
                         # ticks=[-17000,-5,-2.5,0,2.5,5, 16000],
                         cax=dx1,
